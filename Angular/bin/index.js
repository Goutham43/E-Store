...
exports.CheckoutController = function($scope, $user, $http) {
  ...
  // For checkout
  Stripe.setPublishableKey('pk_test_Tq0ENoM3ED0FBX1L1hCvvO9B');
  ...
  $scope.checkout = function() {
    ...
    Stripe.card.createToken($scope.stripeToken, function(status, response) {
     ...
     $http.
        post('/api/v1/checkout', { stripeTokenId: response.id }).
        success(function(data) {
...
app.config(function($routeProvider) {
  $routeProvider.
    when('/category/:category', {
      templateUrl: '/Angular/templates/category_view.html'
    }).
    when('/checkout', {
